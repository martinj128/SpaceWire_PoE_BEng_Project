...
void receiveSWPackets(esp_eth_handle_t eth_handle, uint8_t *eth_rx_buffer, uint32_t len, void *priv)
{
    ...
    for (int i = 0; i < 6; i++) //get MAC destination address from payload
    {
        ...
        { //MAC address byte is correct, do nothing
            ...
            return; //Destination Address not correct, exit function
    }
...
void sendSWPacket(uint8_t *tx_buffer, uint16_t buffer_len, uint8_t sw_packet_type, uint8_t *destination_addr)
{
    ...
    else
    {
        ...
        /*char *char_buffer = convertToCharArray(sw_tx_buffer, (tx_index + 1));
        ESP_LOGI(TAG, "Ethernet Sent %d bytes Containing:\nDest Addr: %02x:%02x:%02x:%02x:%02x:%02x\nSource Addr: %02x:%02x:%02x:%02x:%02x:%02x\nPayload: %s", (tx_index + 1),
                 sw_tx_buffer[0], sw_tx_buffer[1], sw_tx_buffer[2], sw_tx_buffer[3], sw_tx_buffer[4], sw_tx_buffer[5], sw_tx_buffer[6],
                 sw_tx_buffer[7], sw_tx_buffer[8], sw_tx_buffer[9], sw_tx_buffer[10], sw_tx_buffer[11], char_buffer);*/
    }
}

char *convertToCharArray(uint8_t *buffer, uint16_t len)
{
    char *char_buffer = (char *)malloc(len + 1);
    int size = 0;

    for (int i = 0; i < len - 12; i++)
    {
        char_buffer[i] = (unsigned char)buffer[i + 12]; //convert uint8_t to char array to print
        size++;
    }
