...
uint8_t hello[12] = {'h','e','l','l','o',' ','w','o','r','l','d','!'};

struct ethFrame
{
    uint8_t* dest_addr; //mac addr of master
    uint8_t* source_addr; //mac addr of slave
    uint16_t len;
    uint8_t* data;
};
...
void sendTask(void *pvParameters)
{
    while(1)
    {
        sendSWPacket((uint8_t *)hello, sizeof(hello), EOP_HEADER, mac_dest_addr);
